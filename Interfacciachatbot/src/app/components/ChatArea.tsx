...
import { Copy, Paperclip, Send, Check, ThumbsUp, ThumbsDown, RotateCcw, Smile } from 'lucide-react';
              ...
              <div className="flex flex-col gap-1 min-w-0">
                <div className={`group relative ${botBubbleBg} ${botBubbleText} text-[13px] md:text-[14px] leading-relaxed p-3 md:p-4 rounded-[12px] rounded-bl-[4px] border shadow-sm`}>
                  ...
                  <div className="absolute -bottom-8 left-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <AnimatePresence>
                      {msg.liked !== null && msg.liked !== undefined && (
                        <motion.span
                          initial={{ opacity: 0, x: -4 }}
                          animate={{ opacity: 1, x: 0 }}
                          exit={{ opacity: 0 }}
                          className={`text-[9px] font-medium ml-1.5 ${msg.liked ? 'text-[#10B981]' : 'text-[#F73C1C]'}`}
                        >
                          {msg.liked ? 'Feedback positivo' : 'Feedback negativo'}
                        </motion.span>
                      )}
                    </AnimatePresence>
                  </div>
